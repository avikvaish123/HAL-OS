...
#define INT_16_GATE         0x6
#define TRAP_16_GATE        0x7
#define INT_32_GATE         0xE
#define TRAP_32_GATE        0xF
...
/* load directory pointer to CR3 register in preparation for paging.  
In actuality, CR3 only uses bits 31-12 for the address, so only the top 
20 bits of the directory array will be used, which is fine since this 
breaks up entire 32 bit address space into 4KB addressable pages*/
extern void loadPageDirectory(unsigned int*);

/* Enables paging and protected mode by modifying CR0 bits and 
enables 4MB pages through the Page Size Extension (PSE) bit in CR4*/
...
typedef union idt_desc_t {
    ...
    struct {
        ...
        uint8_t  reserved4;
        uint32_t reserved3 : 1; // [size, reserved1, reserved2, reserved3] = gate type
...
/* descriptor used to load the GDTR */
